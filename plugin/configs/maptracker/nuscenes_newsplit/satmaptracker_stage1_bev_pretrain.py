...
num_gpus = 2
batch_size = 3
num_iters_per_epoch = 9274 // (num_gpus * batch_size)  # 1/3 split: 9274 samples
num_epochs = 18
...
model = dict(
    ...
    backbone_cfg=dict(
        ...
        transformer=dict(
            ...
            encoder=dict(
                type='BEVFormerEncoderWithSat',
                num_layers=2,
                pc_range=pc_range,
                num_points_in_pillar=4,
                return_intermediate=False,
                transformerlayers=dict(
                    type='BEVFormerLayerWithSat',
                    sat_gate_init=-3.0,
                    attn_cfgs=[
                        dict(
...
test_pipeline = [
    ...
    dict(type='PadMultiViewImages', size_divisor=32),
    dict(type='FormatBundleMap'),
    dict(type='Collect3D', keys=['img', 'sat_img'], meta_keys=(
        'token', 'ego2img', 'sample_idx', 'ego2global_translation',
        'ego2global_rotation', 'img_shape', 'scene_name'))
]
...
eval_config = dict(
    ...
    pipeline=[
        ...
        dict(type='FormatBundleMap'),
        dict(type='Collect3D', keys=['vectors', 'semantic_mask'], meta_keys=[
            'token', 'ego2img', 'sample_idx', 'ego2global_translation',
            'ego2global_rotation', 'img_shape', 'scene_name'])
    ],
    interval=1,
