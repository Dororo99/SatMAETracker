...
def extract_gt_for_frame(nusc_map, map_explorer, location, lidar2global_translation,
    ...
    for layer_name in ['lane_divider', 'road_divider']:
        ...
        for record in records:
            ...
            # rotate -90 to match MapTracker convention
            new_line = affinity.rotate(new_line, -90, origin=(0, 0), use_radians=False)
            for single_line in _split_collections(new_line):
                if single_line.geom_type != 'LineString':
                    continue
                coords = np.array(single_line.coords)
                simplified = single_line.simplify(0.2, preserve_topology=True)
                coords_s = np.array(simplified.coords)[:, :2]
                label_id = CAT2ID['divider']
                gt_by_label[label_id].append(coords_s)
                instance_id = f"{layer_name}_{record['token']}"
                gt_instances.append((instance_id, label_id, coords_s))

    # --- Boundary (road_segment + lane -> exterior contours) ---
    polygon_list = []
    boundary_tokens = []
        ...
        for record in records:
            ...
            if polygon.is_valid:
                ...
                if not new_polygon.is_empty:
                    ...
                    boundary_tokens.append(f"boundary_{layer_name}_{record['token']}")
    ...
    if polygon_list:
        ...
        label_id = CAT2ID['boundary']
        for i, bl in enumerate(boundary_lines):
            simplified = bl.simplify(0.2, preserve_topology=True)
            coords_s = np.array(simplified.coords)[:, :2]
            gt_by_label[label_id].append(coords_s)
            # boundary is union of multiple records; use index-based ID
            inst_id = f"boundary_line_{i}"
            gt_instances.append((inst_id, label_id, coords_s))
    ...
    label_id = CAT2ID['ped_crossing']
    for i, pcl in enumerate(ped_crossing_lines):
        simplified = pcl.simplify(0.2, preserve_topology=True)
        coords_s = np.array(simplified.coords)[:, :2]
        gt_by_label[label_id].append(coords_s)
        inst_id = f"ped_crossing_line_{i}"
        gt_instances.append((inst_id, label_id, coords_s))
...
def load_all_gt(ann_file, dataroot, roi_size):
    ...
    for sample in samples:
        ...
        # Compute the pose matrix used by MapTracker (lidar-shifted ego)
        lidar_shifted_e2g_translation = np.array(sample['e2g_translation']).copy()
        ...
        pose_4x4 = np.eye(4)
        pose_4x4[:3, :3] = Quaternion(e2g_rotation).rotation_matrix
        pose_4x4[:3, 3] = lidar_shifted_e2g_translation
...
def compute_localization(cur_pred, aligned_prev_pred, roi_size=ROI_SIZE):
    """Localization stability via polyline IoU (x-sampling, y-deviation).
    x_samples are taken from cur_pred only (matching MapStableTest)."""
    if len(cur_pred) == 0 or len(aligned_prev_pred) == 0:
        return -1.0
    half_y = roi_size[1] / 2.0
    # Sample x from current prediction only (not union range)
    ...
    iou = 1.0 - total_diff / (len(x_samples) * half_y)
...
def assign_preds_to_gt(pred_vectors, pred_scores, gt_polylines):
    """
    Assign predictions to GT instances via chamfer distance (greedy).

    Returns:
        list of (matched_pred_vector, matched_score) per GT, or (None, 0.0) if unmatched.
    """
    n_gt = len(gt_polylines)
    if n_gt == 0:
        return []

    results = [(None, 0.0)] * n_gt
    ...
    dist_matrix = chamfer_distance_batch(pred_interp, gt_interp)  # (n_pred, n_gt)

    # Greedy assignment (score-descending)
    scores_arr = np.array(pred_scores)
    sort_inds = np.argsort(-scores_arr)
    gt_assigned = np.zeros(n_gt, dtype=bool)

    for pred_idx in sort_inds:
        if gt_assigned.all():
            break
        dists = dist_matrix[pred_idx]
        # find closest unassigned GT
        for gt_idx in np.argsort(dists):
            if not gt_assigned[gt_idx]:
                gt_assigned[gt_idx] = True
                results[gt_idx] = (np.array(pred_vectors[pred_idx]), float(scores_arr[pred_idx]))
                break
...
def _process_single_pair(args):
    """Process a single frame pair for mAS (worker function for multiprocessing)."""
    ...
    cur_inst_dict = {inst_id: (label, coords) for inst_id, label, coords in cur_gt_inst}
    prev_inst_dict = {inst_id: (label, coords) for inst_id, label, coords in prev_gt_inst}
    ...
    for label_id in ID2CAT.keys():
        label_common = [(iid, cur_inst_dict[iid][1]) for iid in common_ids
                        if cur_inst_dict[iid][0] == label_id]
        if not label_common:
            continue

        gt_ids_ordered = [x[0] for x in label_common]
        gt_coords_cur = [cur_inst_dict[iid][1] for iid in gt_ids_ordered]
        gt_coords_prev = [prev_inst_dict[iid][1] for iid in gt_ids_ordered]

        cur_pred_vecs, cur_pred_scores = [], []
        for i in range(len(cur_pred.get('labels', []))):
            if cur_pred['labels'][i] == label_id:
                cur_pred_vecs.append(cur_pred['vectors'][i])
                cur_pred_scores.append(cur_pred['scores'][i])

        prev_pred_vecs, prev_pred_scores = [], []
        for i in range(len(prev_pred.get('labels', []))):
            if prev_pred['labels'][i] == label_id:
                prev_pred_vecs.append(prev_pred['vectors'][i])
                prev_pred_scores.append(prev_pred['scores'][i])

        cur_assignments = assign_preds_to_gt(cur_pred_vecs, cur_pred_scores, gt_coords_cur)
        prev_assignments = assign_preds_to_gt(prev_pred_vecs, prev_pred_scores, gt_coords_prev)

        cat_name = ID2CAT[label_id]
        for j in range(len(gt_ids_ordered)):
            cur_matched_vec, cur_score = cur_assignments[j]
            prev_matched_vec, prev_score = prev_assignments[j]
            ...
            if cur_matched_vec is not None and prev_matched_vec is not None:
                ...
                ls = compute_localization(np.array(cur_matched_vec), filtered_prev, roi_size)
                ss = compute_shape(np.array(cur_matched_vec), filtered_prev)
            else:
                ls = 0.0
                ss = 0.0
